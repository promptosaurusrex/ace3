...
class TestSolvePowChallenge:

    @pytest.mark.unit
    def test_solve_known_challenge(self):
        """Verify the solver finds a nonce whose hash meets the difficulty target."""
        from scanner import solve_pow_challenge

        challenge = "test_challenge_abc"
        difficulty = 4
        rounds = 100
        nonce = solve_pow_challenge(challenge, difficulty, rounds)

        # Verify the solution independently
        target = (1 << 32) // (1 << difficulty)
        h = hashlib.sha256((challenge + str(nonce)).encode()).digest()
    ...
    def test_solve_difficulty_1(self):
        """Low difficulty should solve very quickly (target = 2^31)."""
        from scanner import solve_pow_challenge

        nonce = solve_pow_challenge("easy", 1, 10)
        assert isinstance(nonce, int)
        assert nonce >= 0

    @pytest.mark.unit
    def test_solve_returns_int(self):
        from scanner import solve_pow_challenge

        nonce = solve_pow_challenge("challenge", 2, 50)
        assert isinstance(nonce, int)

    @pytest.mark.unit
    def test_solve_deterministic(self):
        """Same inputs should produce the same nonce."""
        from scanner import solve_pow_challenge

        n1 = solve_pow_challenge("deterministic_test", 3, 100)
        n2 = solve_pow_challenge("deterministic_test", 3, 100)
        assert n1 == n2


# ---------------------------------------------------------------------------
# Console API handler (PoW params)
# ---------------------------------------------------------------------------

class TestConsoleApiHandler:

    @pytest.mark.unit
    def test_console_handler_pow_params(self, scanner):
        event = MagicMock()
        event.type_ = "log"
        arg = MagicMock()
        arg.value = '__POW_PARAMS__:{"challenge":"abc123","difficulty":6,"rounds":10000}'
        event.args = [arg]

        asyncio.get_event_loop().run_until_complete(scanner.console_api_handler(event))
        assert scanner._pow_params is not None
        assert scanner._pow_params["challenge"] == "abc123"
        assert scanner._pow_params["difficulty"] == 6
        assert scanner._pow_params["rounds"] == 10000

    @pytest.mark.unit
    def test_console_handler_ignores_non_pow(self, scanner):
        event = MagicMock()
        event.type_ = "log"
        arg = MagicMock()
        arg.value = "some random console message"
        event.args = [arg]

        asyncio.get_event_loop().run_until_complete(scanner.console_api_handler(event))
        assert scanner._pow_params is None

    @pytest.mark.unit
    def test_console_handler_ignores_non_log(self, scanner):
        event = MagicMock()
        event.type_ = "warning"
        event.args = []
    ...
    @pytest.mark.unit
    def test_console_handler_pow_challenge(self, scanner, capsys):
        event = MagicMock()
        event.type_ = "log"
        arg = MagicMock()
        arg.value = '__POW_CHALLENGE__:{"cid":"c1","nonce":"n1","difficulty":6}'
        event.args = [arg]

        asyncio.get_event_loop().run_until_complete(scanner.console_api_handler(event))
        captured = capsys.readouterr()
        assert "PoW API challenge received" in captured.out

    @pytest.mark.unit
    def test_console_handler_blob_replaced(self, scanner, capsys):
        event = MagicMock()
        event.type_ = "log"
        arg = MagicMock()
        arg.value = "__POW_BLOB_REPLACED__"
        event.args = [arg]

        asyncio.get_event_loop().run_until_complete(scanner.console_api_handler(event))
        captured = capsys.readouterr()
        assert "PoW Worker blob replaced" in captured.out


# ---------------------------------------------------------------------------
# STEALTH_JS contains PoW intercept code
# ---------------------------------------------------------------------------

class TestStealthJsPowIntercept:

    @pytest.mark.unit
    def test_stealth_js_contains_fetch_intercept(self):
        from scanner import STEALTH_JS

        assert "__POW_CHALLENGE__" in STEALTH_JS
        assert "api=1" in STEALTH_JS

    @pytest.mark.unit
    def test_stealth_js_contains_blob_patch(self):
        from scanner import STEALTH_JS

        assert "__POW_BLOB_REPLACED__" in STEALTH_JS
        assert "crypto.subtle.digest" in STEALTH_JS

    @pytest.mark.unit
    def test_stealth_js_contains_worker_intercept(self):
        from scanner import STEALTH_JS

        assert "__powWorkers" in STEALTH_JS
        assert "__POW_PARAMS__" in STEALTH_JS
